...
| 能力 | 说明 |
|------|------|
| **多模型** | OpenAI 兼容、Ollama、Claude、Gemini 等（以各接口能力为准）；支持流式输出（与模型/接口能力相关） |
| **会话** | 多会话、重命名、删除、未读提醒、导出整段会话为 **Markdown / HTML** |
| **附件** | 图片多模态（视模型支持）、**Excel / Word / Markdown / 文本** 等：主进程解析后注入对话供模型阅读 |
| **联网** | 关键词或 `/web` 等触发；免 Key 走 DuckDuckGo 等链路，可选 **Tavily / Brave**（需 API Key） |
| **工作区** | 在设置中配置目录后，可自动节选 `MYAGENT_KNOWLEDGE.md` / `knowledge.md` / `README.md` 作上下文 |
| **界面** | **中文 / 英文** 切换、亮 / 暗 / **跟随系统**；新用户引导（可跳过） |
| **助手消息** | 使用 **GitHub 风格 Markdown** 渲染（含**表格**）；单条回复可**另存为 .md / .xlsx / .docx**（xlsx 由回复中的管道表转换） |
| **工具与自动化** | 生图、启动本机应用、剪贴板联动等（见应用内能力） |

### 文档与表格说明

- 上传 **`.xlsx` / `.docx` / `.md` 等** 后，系统会将可读文本或表格的 Markdown 表示附在发送内容后发给模型。  
...
- 对话与设置主要保存在本机 **应用数据目录**（与 Electron `userData` 一致，开发版与安装版共用同一机上的数据策略以实际为准）。  
