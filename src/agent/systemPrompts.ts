...
export function buildAgentCapabilitySystem(
  ...
  if (locale === 'en') {
    ...
    'For finding EXISTING photos on disk, use local_search with mode "image" — do NOT use AI image generation. ' +
      ...
      'Baidu image search: https://image.baidu.com/search/index?tn=baiduimage&word=KEYWORD — use web_eval to read img src/href for the first result when user asks for an image.\n' +
      ...
      `${ragHint}\n` +
      'You have direct disk + browser access. Do NOT ask the user to upload or to operate the browser themselves; perform actions yourself via web_eval. ' +
  ...
  return (
    ...
    '百度图片搜索：https://image.baidu.com/search/index?tn=baiduimage&word=关键词 ；用户要「第一张图」时用 web_eval 取结果区 img 的 src/data-img 或链接。\n' +
    ...
    `${ragHint}\n` +
    '你已具备本机磁盘直读 + 浏览器操作能力。禁止要求用户提供文档名/上传/自行点击浏览器；需要在网页上做的事请自己用 web_eval 完成。' +
